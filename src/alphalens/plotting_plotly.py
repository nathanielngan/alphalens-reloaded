...
PLOT_HEIGHT = 500
PLOT_WIDTH = 1000
PLOT_COLOR = px.colors.qualitative.D3

theme = {'header1' : '#FFFFFF',
         'header2' : '#2fe1ee',
         'header3': '#00A3E0',
         # 'text' : '#00A3E0',
         'text': '#041E42',
         # 'background' : '#041E42',
         'layout': {'title_font_color' : '#00A3E0',
                    # 'paper_bgcolor' : '#041E42',
                    'plot_bgcolor' : '#ededed',
                    # 'font_color' : '#00A3E0',
                    'font_color' : '#041E42',
                    'font_size' : 16,
                    'legend_font_color' : '#00A3E0',
                    'legend_font_size' : 12
                    }
}

def plotly_table_fig(table, id= '', title_text=None):
    fig = go.Figure(data=[
        go.Table(
        ...
        cells=dict(values=table.round(4).T.values,
                   # fill=dict(color=['#bebebe', 'white']),
                   align=['left', 'center'],
                   height=30,
                   fill_color=[['#FFFFFF', '#bebebe'] * 8],
                   line_color='darkslategray'
                   ))],
        layout=go.Layout({'paper_bgcolor': theme.get('background', None),
                          'font_color': theme.get('text'),
                          'font_size': 14,
                          'height': 450})
    )
...
def plot_returns_table(
    ...
    if return_df:
        fig = plotly_table_fig(returns_table.reset_index(),
                               id ='returns_table',
                               title_text= 'Return Analysis')
...
def plot_turnover_table(autocorrelation_data, quantile_turnover, return_df=False):
    ...
    if return_df:
        turnover_fig = plotly_table_fig(table=turnover_table.reset_index(),
                                        title_text='Turnover Table',
                                        id= 'turnover_table')
        auto_corr_fig = plotly_table_fig(table=auto_corr.reset_index(),
                                         title_text='Auto Corr. Analysis',
                                         id='auto_corr_table')
        return [turnover_fig, auto_corr_fig]
    else:
...
def plot_ic_ts(ic, ax=None):
    ...
    for i in ic:
        ...
        fig.update_layout(theme.get('layout'))
        ax.append(dcc.Graph(id='ic_ts_plot' + i, figure=fig))
...
def plot_quantile_returns_bar(
    ...
    if by_group:
        ...
        #TODO: Change this to plotly

        # fig = px.bar(mean_ret_by_q.loc['Ann. Rets'],
        #              labels={"value": "Percentage %"})
        # fig.update_layout(theme.get('layout'))
        # f = dcc.Graph(id='avg_ret_bar_chart', figure=fig)


        #This is the plot
        fig = px.bar(mean_ret_by_q.multiply(DECIMAL_TO_BPS),
                     title= "Mean Period Wise Return By Factor Quantile",
                     labels={'value': "Mean Return (bps)", "factor_quantile": "Quantile"},
                     barmode='group',
                     range_y=(ymin, ymax),
                     height = PLOT_HEIGHT,
                     width= PLOT_WIDTH,
        ...
        return dcc.Graph(id='quantile_returns_bar', figure=fig)
...
def plot_quantile_returns_violin(return_by_q, ylim_percentiles=None, ax=None):
    ...
    fig = px.violin(unstacked_dr,
                title="Period Wise Return By Factor Quantile",
                labels={'value': "", "factor_quantile": "Return (bps)"},
                y = 'return',
                x = 'factor_quantile',
                color = 'forward_periods',
                box = True,
                # range_y = (ymin, ymax),
                height=PLOT_HEIGHT,
                width=PLOT_WIDTH,
    ...
    fig.update_layout(theme.get('layout'))
    return dcc.Graph(id='quantile_returns_violin', figure=fig)
    ...
    if isinstance(mean_returns_spread, pd.DataFrame):
        ...
        for i in mean_returns_spread_bps:
            title = (
                ...
                "({} Period Forward Return)".format(i if i is not None else "")
            )
            ...
            fig = px.line(tmp,
                          ...
                          color_discrete_sequence=PLOT_COLOR)
            fig.update_layout(theme.get('layout'))
            ax.append(dcc.Graph(id='cumulative_returns_by_quantile' + i, figure=fig))


            # color = "forestgreen"
            #Need a rolling average
            # color = "orangered"
            # ax.legend(["mean returns spread", "1 month moving avg"], loc="upper right")

    # mean_returns_spread_bps.plot(alpha=0.4, ax=ax, lw=0.7, )
...
def plot_factor_rank_auto_correlation(factor_autocorrelation, period=1, ax=None):
    ...
    fig = px.line(factor_autocorrelation.fillna(0),
                  ...
                  title=title,
                  range_x=(factor_autocorrelation.index[0],factor_autocorrelation.index[-1]),
                  height=PLOT_HEIGHT,
                  width=PLOT_WIDTH,
                  color_discrete_sequence=PLOT_COLOR)
    fig.add_annotation(x=1, y=1,
                       text= "\t\t Mean Autocorrelation %.3f" % factor_autocorrelation.mean(),
                       showarrow=False,
                       xshift=5)

    fig.update_layout(theme.get('layout'))

    return dcc.Graph(id='factor_rank_auto_correlation_plot'+str(period), figure=fig)
...
def plot_top_bottom_quantile_turnover(quantile_turnover, period=1, ax=None):
    ...
    return dcc.Graph(id='plot_top_bottom_quantile_turnover'+str(period), figure=fig)
...
def plot_monthly_ic_heatmap(mean_monthly_ic, ax=None):
    ...
    for i in mean_monthly_ic:
        tmp = mean_monthly_ic.iloc[:,0]
        tmp = pd.DataFrame({'Year': tmp.index.year.values,
                            'Month': tmp.index.month.values,
                            'Values':tmp.values})
        tmp = tmp.set_index(['Year', 'Month']).unstack()
        title = "Monthly Mean {} Period IC".format(i)
        fig = px.imshow(tmp.values.round(2),
                        labels=dict(x="Month", y="Year"),
                        color_continuous_scale=["red","white","green"],
                        x=tmp.columns.get_level_values(1).tolist(),
                        y=tmp.index.tolist(),
                        text_auto=True,
                        title=title,
                        width=PLOT_WIDTH,
        ...
        fig.update_yaxes(tick0=tmp.index.min(), dtick=1)
        fig.update_xaxes(tick0=1, dtick=1)
        fig.update_layout(theme.get('layout'))
        ax.append(dcc.Graph(id='monthly_ic_heatmap_plot' + i, figure=fig))
...
def plot_cumulative_returns(factor_returns, period, freq=None, title=None, ax=None):
    ...
    return dcc.Graph(id='cumulative_returns', figure=fig)
...
def plot_cumulative_returns_by_quantile(quantile_returns, period, freq=None, ax=None):
    ...
    return dcc.Graph(id='cumulative_returns_by_quantile', figure=fig)
